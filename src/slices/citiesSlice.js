...
const citiesSlice = createSlice({
  ...
  reducers: {
    generateLeague: (state, action) => {
      const { id } = action.payload;

      // Find the user team and other teams
      const userTeam = state.find((city) => city.cityID === id);
      const otherTeams = state.filter((city) => city.cityID !== id);

      const shuffledTeams = otherTeams.sort(() => Math.random() - 0.5);
      const selectedTeams = shuffledTeams.slice(0, 29);

      // Add roster and set hasTeam to true for the user's team
      userTeam.userTeam = true;
      userTeam.hasTeam = true;
      userTeam.roster = {
        PG: null,
        SG: null,
        SF: null,
      ...
      userTeam.scoutedCities = [
        { cityID: userTeam.cityID, location: userTeam.location },
      ];

      // Loop through 30 teams that are not users team && set has team to true and add roster
      selectedTeams.forEach((team) => {
        ...
        team.scoutedCities = [{ cityID: team.cityID, location: team.location }];
      });

      state.map((team) =>
        selectedTeams.some(
          (selectedTeam) =>
            selectedTeam.id === team.id || userTeam.id === team.id
        )
          ? {
              ...team,
              hasTeam: true,
    ...
    addUserInfo: (state, action) => {
      return state.map((city) => {
        let { chosenCity, players } = action.payload;
        let distance = calculateDistance(chosenCity.location, city.location);
        let playersInRange = getPlayersInRange(
          city.location,
          players,
          chosenCity.scouting
        );
        return {
          ...
          userInfo: {
            distance: distance,
            estimatedPlayersInRange: playersInRange,
            scoutTime: calculateScoutTime(distance),
          },
        };
      });
    },
    addScoutedCity: (state, action) => {
      const { id, city } = action.payload;

      return state.map((currentCity) =>
    ...
    rosterPlayer: (state, action) => {
      const { teamID, player } = action.payload;
      return state.map((team) => {
        if (team.cityID === teamID) {
          console.log("ADDING TO ROSTER", team.name, player.name);
          return {
            ...team,
            roster: {
              ...team.roster,
    ...
    addUserPlayer: (state, action) => {
      const { teamID, player } = action.payload;

      console.log("In here with", teamID, player);

      const updatedState = state.map((team) =>
        team.cityID === teamID
          ? { ...team, roster: addPlayerToRoster(team.roster, player) }
          : team
      );
    ...
    swapPlayers: (state, action) => {
      const { teamID, pos1, pos2 } = action.payload;

      return state.map((team) =>
        team.cityID === teamID
          ? { ...team, roster: swapPositions(team.roster, pos1, pos2) }
          : team
      );
    },
