...
function handleSimulation(cities, trips, contracts, players, day) {
  // New Trips Started
  ...
  tripsNeeded.forEach((team) => {
    ...
    let trip = createTrip(team, randomCity, day, trips);
    trips.push(trip);
  });
  ...
  leagueCities = cities.filter((city) => city.hasTeam && !city.userTeam);
  ...
  return { cities, trips, contracts, players };
}

export function local_handleNewTrips(cities, trips, day) {
  // New Trips Started
  let leagueCities = cities.filter((city) => city.hasTeam && !city.userTeam);
...
export function local_handleFinishedTrips(trips, cities, day) {
  let newTrips = [...trips];
...
export function local_handleFinishedContracts(contracts, players, day, cities) {
  // Started Contracts Finished
  const finishedContracts = contracts.filter(
    (contract) => contract.finishDay === day && !contract.isHandled
  );
...
export function local_handleNewContracts(
  cities,
  contracts,
  players,
  playerIds,
...
export function local_finishSimulation(trips, cities, contracts, players, day) {
  let newTrips = local_handleNewTrips(cities, trips, day);
  ...
  let { finishedContracts, finishedPlayers, newerCities, playerIds } =
    local_handleFinishedContracts(contracts, players, day, newCities);
